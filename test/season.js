...
describe('Season', () => {
  ...
  describe('bounds', () => {
    ...
    it('max', () => {
      ...
      expect(new Season(1943, 24).max)
        .to.eql(Date.UTC(1943, 11, 31, 23, 59, 59, 999))
    })

    it('max should equal next.min - 1', () => {
      let seasons = [
        new Season(2016, 21),
        new Season({ values: [2016, 21], unspecifed: 'yyyxmmdd' }),
      ]

      for (let s of seasons)
        expect(s.max).to.eql(s.next().min - 1)
